...
import { VerificationBadge } from '@/components/VerificationBadge'; // Certifique-se que o caminho está certo
...
export default function PostCard({ post, currentUserId, onCommentClick }: Props) {
  ...
  const handleLike = async (e: React.MouseEvent) => {
    ...
    try { await togglePostLike(post.id, liked); } catch { setLiked(!newState); setLikesCount(prev => !newState ? prev + 1 : prev - 1); }
  };
  ...
  return (
    <article 
        className="border-b border-gray-100 px-4 py-4 hover:bg-gray-50/30 transition-colors cursor-pointer bg-white w-full animate-in fade-in"
      ...
      <div className="flex gap-3 w-full">
         ...
         <div className="flex-shrink-0">
            <Link href={`/u/${post.user.username.replace('@', '')}`} onClick={e => e.stopPropagation()}>
                <div className="relative w-10 h-10 rounded-full bg-gray-200 overflow-hidden border border-gray-100 hover:opacity-90">
         ...
         <div className="flex-1 min-w-0">
            ...
            {/* Header com Verificação Corrigida */}
            <div className="flex justify-between items-start">
                <div className="flex items-center flex-wrap gap-x-1 text-[15px] leading-5">
                    {/* Nome + Badge em um container flex */}
                    <div className="flex items-center gap-1 max-w-full">
                        <Link href={`/u/${post.user.username}`} onClick={e => e.stopPropagation()} className="font-bold text-gray-900 hover:underline truncate">
                            {post.user.name}
                        </Link>
                        {/* O badge agora está colado ao nome */}
                        {post.user.badge && (
                    ...
                    <span className="text-gray-500 text-sm ml-0.5">@{post.user.username}</span>
                ...
                <div className="relative group/menu ml-2">
                    ...
                    {isMenuOpen && (
                        <div className="absolute right-0 top-6 bg-white shadow-lg border border-gray-100 rounded-lg z-50 w-32 py-1">
                            {isOwner ? (
            ...
            <div className="flex justify-between items-center mt-3 max-w-[425px] text-gray-500">
                ...
                <button onClick={handleShare} className="group flex items-center gap-2 hover:text-green-500 transition-colors">
