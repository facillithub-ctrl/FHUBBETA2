    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
        <div className="bg-white dark:bg-dark-card rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden border border-gray-200 dark:border-dark-border">
            <div className="bg-gradient-to-r from-[#42047e] to-[#07f49e] p-6 text-white flex justify-between items-center">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs opacity-90">Revise o feedback antes de salvar.</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                    {feedback.detailed_feedback?.map((item, i) => (
                        <div key={i} className="bg-white dark:bg-gray-800 p-4 rounded-xl border-l-4 border-[#42047e] shadow-sm">
                            <h5 className="font-bold text-sm text-[#42047e] dark:text-[#07f49e] mb-1">{item.competency}</h5>
            ...
            <div className="p-4 border-t border-gray-100 dark:border-gray-700 bg-white dark:bg-dark-card flex justify-end gap-3">
                <button onClick={onCancel} className="px-6 py-2.5 rounded-xl text-sm font-bold text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">Descartar</button>
...
// Card Flutuante de Comentário
const AnnotationDetail = ({ annotation, onClose }: { annotation: Annotation, onClose: () => void }) => (
    <div className="absolute z-50 bg-white dark:bg-dark-card p-4 rounded-xl shadow-2xl border-l-4 border-[#42047e] animate-in fade-in slide-in-from-bottom-2 w-full max-w-sm" style={{ top: '100%', left: 0, marginTop: '10px' }}>
        <div className="flex justify-between items-start mb-2">
            ...
            <button onClick={(e) => { e.stopPropagation(); onClose(); }} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"><i className="fas fa-times"></i></button>
        </div>
...
export default function EssayCorrectionView({ essayId, onBack }: {essayId: string, onBack: () => void}) {
    ...
    // CORREÇÃO AQUI: Tratamento de Erro API
    const handleGenerateAI = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                // Lança o erro vindo do servidor para o catch
                throw new Error(aiData.error || `Erro ${response.status}: Falha na API`);
            }
            ...
            if (addToast) {
                addToast({ type: 'error', message: error.message || 'Erro ao conectar com IA.' });
            } else {
                alert(error.message || "Erro ao conectar com IA.");
            }
        } finally {
    ...
    const handleShare = async () => {
        try { await navigator.share({ title: 'Minha Redação', url: window.location.href }); } catch (e) { if(addToast) addToast({ type: 'info', message: 'Link copiado.' }); }
    };
    const handleContest = () => {
        if (confirm("Deseja contestar esta correção?")) { if(addToast) addToast({ type: 'success', message: 'Enviado para análise.' }); }
    };
    ...
    const renderText = () => {
        ...
        if (viewMode === 'clean') {
             return <div className="whitespace-pre-wrap text-lg leading-relaxed text-gray-800 dark:text-gray-200 font-serif" dangerouslySetInnerHTML={{ __html: details.content }} />;
        }

        const annotations = details.correction?.annotations || [];
        if (annotations.length === 0) {
            const clean = details.content.replace(/<br\s*\/?>/gi, '\n').replace(/<[^>]+>/g, '');
            return <div className="whitespace-pre-wrap text-lg leading-relaxed text-gray-800 dark:text-gray-200 font-serif">{clean}</div>;
        }

        // Quebra por parágrafos para processar anotações sem destruir layout
        const paragraphs = details.content.replace(/<br\s*\/?>/gi, '\n').replace(/<[^>]+>/g, '').split('\n');

        return (
            <div className="space-y-4 text-lg leading-relaxed text-gray-800 dark:text-gray-200 font-serif select-text">
                {paragraphs.map((paragraph, pIdx) => {
                    if (!paragraph.trim()) return <br key={pIdx} />;

                    const paragraphAnnotations = annotations.filter(a => a.selection && paragraph.includes(a.selection));

                    if (paragraphAnnotations.length === 0) return <p key={pIdx}>{paragraph}</p>;

                    let parts: ReactNode[] = [paragraph];

                    paragraphAnnotations.forEach(anno => {
                            ...
                            if (typeof part === 'string') {
                                ...
                                split.forEach((s, i) => {
                                    ...
                                    if (i < split.length - 1) {
                                        ...
                                        const colorClass = anno.marker === 'erro' ? 'bg-red-200 border-b-2 border-red-500' : anno.marker === 'acerto' ? 'bg-green-200 border-b-2 border-green-500' : 'bg-blue-200 border-b-2 border-blue-500';
                                        
                                        nextParts.push(
                                            <span 
                                                key={`${anno.id}-${i}`}
                                                onClick={(e) => { e.stopPropagation(); setActiveAnnotationId(isActive ? null : anno.id); }}
                                                className={`relative cursor-pointer px-0.5 transition-all rounded hover:opacity-80 ${colorClass} ${isActive ? 'ring-2 ring-[#42047e]' : ''}`}
                                            >
                                                {anno.selection}
                                            </span>
                                        );
    ...
    if (isLoading) return <div className="h-96 flex items-center justify-center"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#42047e]"></div></div>;
    if (!details) return <div className="p-10 text-center text-red-500">Erro ao carregar.</div>;
    ...
    return (
        <div className="animate-fade-in pb-20 relative" onClick={() => setActiveAnnotationId(null)}>
            ...
            <div className="sticky top-0 z-50 bg-white/90 dark:bg-dark-card/90 backdrop-blur-lg border-b border-gray-200 dark:border-dark-border px-6 py-3 flex justify-between items-center shadow-sm transition-all">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-sm font-bold text-gray-800 dark:text-white hidden md:block truncate max-w-md">{details.title || "Correção"}</h1>
                </div>

                <div className="flex items-center gap-3">
                    <div className="hidden md:flex bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
                        <button onClick={() => setViewMode('corrected')} className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${viewMode === 'corrected' ? 'bg-white shadow text-[#42047e] dark:bg-gray-700 dark:text-white' : 'text-gray-500'}`}>Correção</button>
                        <button onClick={() => setViewMode('clean')} className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${viewMode === 'clean' ? 'bg-white shadow text-[#42047e] dark:bg-gray-700 dark:text-white' : 'text-gray-500'}`}>Limpo</button>
                    </div>
                    <div className="flex gap-1">
                        <button onClick={handleDownload} className="p-2 text-gray-500 hover:text-green-600 rounded-lg transition-colors"><i className="fas fa-print"></i></button>
                        <button onClick={handleShare} className="p-2 text-gray-500 hover:text-blue-600 rounded-lg transition-colors"><i className="fas fa-share-alt"></i></button>
                        <button onClick={handleContest} className="p-2 text-gray-500 hover:text-red-600 rounded-lg transition-colors"><i className="fas fa-flag"></i></button>
                    </div>
                    
                    {details.correction && !details.correction.ai_feedback && (
                        <button onClick={handleGenerateAI} disabled={isGeneratingAI} className="ml-2 bg-gradient-to-r from-[#42047e] to-[#07f49e] text-white text-xs font-bold px-4 py-2 rounded-lg shadow hover:shadow-lg hover:scale-105 transition-all flex items-center gap-2 disabled:opacity-70">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 p-6 items-start">
                <div className="xl:col-span-7">
                    <div className="bg-white dark:bg-dark-card p-8 rounded-3xl shadow-sm border border-gray-100 dark:border-dark-border min-h-[600px] relative">
                        {activeAnnotation && <div className="sticky top-24 z-30 mb-4" onClick={e => e.stopPropagation()}><AnnotationDetail annotation={activeAnnotation} onClose={() => setActiveAnnotationId(null)} /></div>}
                        {details.image_submission_url ? <Image src={details.image_submission_url} alt="Redação" width={800} height={1200} className="w-full h-auto rounded-xl" /> : renderText()}
                    </div>
                </div>
                ...
                <div className="xl:col-span-5 space-y-6 xl:sticky xl:top-24">
                    <div className="bg-white dark:bg-dark-card p-6 rounded-3xl shadow-lg border border-gray-100 dark:border-dark-border relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-center mt-2">
                            <div><p className="text-xs font-bold text-gray-400 uppercase mb-1">Nota Final</p><div className="flex items-baseline gap-1"><span className={`text-5xl font-black tracking-tighter ${gradeColor}`}>{finalGrade}</span><span className="text-lg text-gray-400 font-bold">/1000</span></div></div>
                            ...
                            {details.correction && competencyDetails.map((comp, i) => {
                                ...
                                return (
                                    <div key={i} className="p-4 hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                                        <div className="flex justify-between mb-1"><span className="text-xs font-bold text-gray-500 uppercase">{comp.title.split(':')[0]}</span><span className="font-black text-dark-text dark:text-white">{grade}</span></div>
                                        <div className="w-full bg-gray-100 rounded-full h-1.5 dark:bg-gray-700"><div className="bg-[#42047e] h-full rounded-full" style={{ width: `${pct}%` }}></div></div>
                                    </div>
                                );
                            })}
                        </div>
                    ...
                    <div className="bg-white dark:bg-dark-card rounded-3xl shadow-sm border border-gray-100 dark:border-dark-border overflow-hidden p-1">
