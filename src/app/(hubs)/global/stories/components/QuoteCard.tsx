...
export default function QuoteCard({ quote }: { quote: QuoteData }) {
  ...
  const themes = {
    ...
    paper: "bg-[#fdfbf7] text-[#2c2c2c] font-serif border border-[#e8e4d9] shadow-sm", 
  ...
  return (
    <div className={`p-8 rounded-2xl relative mb-6 transition-all hover:scale-[1.01] ${themes[quote.theme]} group`}>
      ...
      <div className="absolute bottom-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
         <button onClick={handleCopy} className="w-8 h-8 rounded-full bg-white/20 backdrop-blur-sm hover:bg-white/40 flex items-center justify-center text-current transition-colors">
